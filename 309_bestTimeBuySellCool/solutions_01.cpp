...
public:
    int maxProfit(vector<int>& prices) {
        ...
        vector<int> buy(n,0); // the best profit made if we buy today at i
        vector<int> sel(n,0); // the best profit made if we sell today at i
        vector<int> col(n,0); // the best profit made if we cooldown today at i
        for(int i=1; i<n; i++){
